...
async def get_last_input(tg_id : int):
    query_get = f"SELECT last_input FROM USERS WHERE tg_id = {tg_id}"
...
async def get_img(message: types.Message):
    ...
    queries = []
    queries.append(
        f"INSERT INTO IMAGES (img_path, idea_id) VALUES ('{path}', 1);")
    await insert_data(queries)
...
async def echo(message: types.Message):
    ...
    # MODIFY IDEA BUTTON STATUSES
        ...
        idea_name = await get_last_input(message.from_user.id)
    ...
    # DEFAULT STATUS HANDLER
        ...
        db = _mysql.connect(host="localhost", user=usr,
                            password=pswd, database=dbase)
        db.query("SELECT IF(2>1,'negativ','pozitiv') AS kek")
        query_res = db.store_result()
        query_res = query_res.fetch_row(maxrows=0, how=1)     
        print("\n",query_res)   
