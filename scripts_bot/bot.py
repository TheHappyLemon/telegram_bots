...
async def parse_msg(msg: str, force: bool = False, slash: bool = False, input : bool = False, input_web : bool = False):
    if input:
        if input_web:
            for char in ch_in_web:
...
async def echo(message: types.Message):
    ...
    # MODIFY IDEA BUTTON STATUSES
        ...
        origin_new_in  = await parse_msg(message.text.strip().lower(), input=True, input_web=True) # i think it can be removed...
        origin_new_out = await parse_msg(message.text.strip().lower(), force=True)
        idea_name = await get_custom_column(message.from_user.id)
        idea_name_out = await parse_msg(idea_name, True)
        print("\naaa\n")
        queries.append(
        ...
        queries.append(
            f"UPDATE USERS SET sts_chat = 'IDLE' WHERE tg_id = {message.from_user.id};")
        msg = f"Done\!\nSource of idea *{idea_name_out}* is now *{origin_new_out}*\!"
